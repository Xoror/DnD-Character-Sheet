...
const ConditionsSlice = createSlice({
    ...
    reducers: {
        addCondition(state, action) {
            state.conditionsHas.push(state.conditions.find(condition => (condition.id === action.payload)))
        },
        removeCondition(state, action) {
            state.conditionsHas = state.conditionsHas.filter(condition => (condition.id != action.payload))
        },
